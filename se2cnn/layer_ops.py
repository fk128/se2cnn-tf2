...
import numpy as np
from se2cnn import rotation_matrix
...
def z2_se2n(
    ...
    # channelsOUT axis)
    kernels_as_if_2D = tf.cast(tf.transpose(kernel_stack, [1, 2, 3, 0, 4]), dtype="float32")
...
def spatial_max_pool(input_tensor, nbOrientations, pool=2, padding='VALID'):
...
def rotate_lifting_kernels(kernel, orientations_nb, periodicity=2 * np.pi, diskMask=True):
    ...
    rotOp_matrix = tf.SparseTensor(
        idx, tf.cast(vals, "float32"),
        [orientations_nb * kernelSizeH * kernelSizeW, kernelSizeH * kernelSizeW])

    # Matrix multiplication
    # Resulting shape: [nbOrientations*kernelSizeH*kernelSizeW,
    # channelsIN*channelsOUT]
    set_of_rotated_kernels = tf.sparse.sparse_dense_matmul(
